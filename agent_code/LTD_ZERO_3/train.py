...
import events as e
from .callbacks import action_value_aprox_Function, policy, action_state_to_features, bomb_endangers_nearest_opponent
...
AWAY_FROM_OPPONENT = "AWAY_FROM_OPPONENT"
BOMB_THREATENED_OPPONENT = "BOMB_THREATENED_OPPONENT"
BOMB_DID_NOT_THREATEN_OPPONENT = "BOMB_DID_NOT_THREATEN_OPPONENT"
...
def check_rewards(old_game_state,new_game_state,self_action,events):
    ...
    check_opponent_closer(self_action,old_game_state,events)
    check_bomb_threatens_opponent(self_action,old_game_state,events)
...
def check_opponent_closer(self_action,old_game_state,events):
    new_features = action_state_to_features(old_game_state,self_action)
    others = old_game_state["others"]
    if not others:
        return
    if new_features[8] == 1:
        events.append(CLOSER_TO_OPPONENT)
    else:
        events.append(AWAY_FROM_OPPONENT)

def check_bomb_threatens_opponent(self_action,old_game_state,events):
    _, _, bomb, (x, y) = old_game_state["self"]
    endanger = bomb_endangers_nearest_opponent(old_game_state, x, y)
    features = action_state_to_features(old_game_state,self_action)
    if self_action != 'BOMB' and features[1] == 1 and endanger:
        events.append(BOMB_DID_NOT_THREATEN_OPPONENT)
    features = action_state_to_features(old_game_state,self_action)
    if features[9] == 1 and features[1] == 1:
        events.append(BOMB_THREATENED_OPPONENT)
...
def end_of_round(self, last_game_state, last_action, events):
    ...
    check_moved_to_death(last_game_state,last_action,events)
    check_opponent_closer(last_action,last_game_state,events)
    check_bomb_threatens_opponent(last_action,last_game_state,events)
...
def reward_from_events(self, events: List[str]) -> float:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    game_rewards = {
        ...
        CLOSER_TO_CRATE: 2,
        AWAY_FROM_CRATE: -1,
        CLOSER_TO_OPPONENT: 3,
        AWAY_FROM_OPPONENT: -2,
        BOMB_THREATENED_OPPONENT: 5,
        BOMB_DID_NOT_THREATEN_OPPONENT: -5,
    }
    reward_sum = 0
    for event in events:
